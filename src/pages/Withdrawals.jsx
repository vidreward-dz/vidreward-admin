...
export default function Withdrawals() {
  ...
  async function handleApprove(w) {
    if (!window.confirm(`الموافقة على طلب سحب بقيمة ${w.amount} دج لـ ${w.users?.name ?? "مستخدم"}؟`)) return;
    setBusyId(w.id);
    try {
  ...
  async function handleMarkPaid(w) {
    if (!window.confirm(`تأكيد أنك حوّلتِ ${w.amount} دج فعلياً لـ ${w.phone_number}؟`)) return;
    setBusyId(w.id);
    try {
